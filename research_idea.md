...
A 2-layer MLP head $f_\psi$ (512 → 128 → 10) regresses the pose of the ball and both teams' centroids in the *next* frame:
$$\hat{y}_{t+1} = f_\psi(z_t) = [\hat{x}_{\text{ball}}, \hat{y}_{\text{ball}}, \hat{x}_{\text{a}}, \hat{y}_{\text{a}}, \hat{\theta}_{\text{a}}, \hat{x}_{\text{o}}, \hat{y}_{\text{o}}, \hat{\theta}_{\text{o}}]_{t+1}$$

where $\text{a}$ denotes the agent's team and $\text{o}$ the opponent. Since the game is 2v2 and the agent controls both of its team's players with one touch, we model the team as a single rigid object: $(x_{\text{a}}, y_{\text{a}})$ is the centroid of the two agent players and $\theta_{\text{a}}$ is the angle of the vector between them. Symmetrically for the opponent. Orientation is encoded as $(\sin\theta, \cos\theta)$ pairs in practice to avoid the wrap-around discontinuity, expanding the regression target to 10 dimensions; for clarity of exposition we write $\theta$ above.
