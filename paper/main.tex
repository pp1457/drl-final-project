...
\begin{itemize}
  ...
  \item \textbf{(C2) An offline analysis of the trained actors}
        ...
        holds.
  \item \textbf{(C3) A single-seed RND ablation that causally
        attributes the entropy-collapse phenomenon to RND.} Across
        the 9 RND-on runs we observe at least 3 hard
        entropy-collapse events (policy entropy $<\!0.05$);
        re-running baseline PPO with \texttt{--use-rnd} disabled on
        one seed produces \emph{zero} such events. Final
        deterministic-evaluation return is unchanged ($-1.67$ in
        both conditions), so RND drives the collapses we documented
        but does not change final policy quality at our budget.
...
observe at least 3 hard collapses in the RND-on runs; a 1-seed
RND-off ablation produces zero collapses across the full 312
PPO updates while yielding the same final eval return. RND is
the proximate cause of, not a remedy for, the collapse phenomenon
in this regime, but has no net effect on final performance. The cleanest methodological takeaways are (a) that
