...
uniform-random baseline confirms that all three conditions did
learn \emph{something} above random ($-3.00$ per quarter)---each
trained-condition mean is $1.5$--$2.3$ reward above the random
floor---but the auxiliary-loss conditions do not improve on the
no-aux baseline. We additionally analyse the trained
...
\begin{table}[h]
  \caption{Deterministic evaluation of all 9 final checkpoints
  ($3$ episodes $\times$ $96$-step cap per checkpoint). Each
  return is the cumulative HOU--CHI score over one quarter. Bold
  mean indicates the best per-condition mean.}
  \label{tab:eval}
  \centering
  \small
  \begin{tabular}{lcccc}
    ...
    \textbf{Condition} & \textbf{Seed 0} & \textbf{Seed 1} & \textbf{Seed 2} & \textbf{Mean (std)} \\
    \midrule
    Baseline & ws1: $-1.67$ ($[-2,0,-3]$) & ws2: $\phantom{-}0.00$ ($[-1,0,+1]$) & ws3: $-0.33$ ($[0,-1,0]$) & $\mathbf{-0.67\;(0.85)}$ \\
...
\textbf{(i) All three trained conditions are clearly above the
uniform-random baseline.} As a control we evaluated a
uniform-random policy under the identical protocol ($3$ episodes
$\times$ $96$-step cap on the same emulator and snapshot) and
measured a mean return of $-3.00$ (raw $\{-3, -5, -1\}$, std
$2.00$). All three trained-condition means
(baseline $-0.67$, OCA $-1.44$, DPR $-0.89$) sit
$1.5$--$2.3$ reward above this random-policy floor, so
\emph{some} learning has happened: even though the training-time
plot of Figure~\ref{fig:traincurves} appears to plateau, the
deterministic $\arg\max$ policy that PPO produced is meaningfully
better than uniform random. The training-time metric is computed
over a $\sim$$10\times$ longer horizon (full-game $1024$-step
episodes) and over the stochastic on-policy samples PPO actually
trained on (rather than the deterministic policy evaluated here),
which is why training-time returns of $\approx-19$ do not
contradict per-quarter eval returns of $\approx-1$.
...
shows that all three conditions are clearly above the
measured uniform-random baseline ($\approx -3.0$ per 96-step
episode), but the auxiliary-loss conditions do not improve on the
no-aux baseline: baseline-PPO numerically out-performs both OCA
($-0.67$ vs $-1.44$) and DPR ($-0.67$ vs $-0.89$), with
per-condition gaps comparable to or smaller than inter-seed std.
Neither auxiliary loss provides a detectable benefit in this
setting. To \textbf{(Q2)}---does our charge-duration observation
