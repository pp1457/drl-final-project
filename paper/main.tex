...
empirical study of auxiliary-representation losses
...
the deterministic in-game CPU opponent. Per-seed final
training-time returns cluster tightly around $-19$ (HOU loses by
$\approx 19$ baskets per $4$-quarter game), with per-condition
means within $0.41$ reward of each other and inter-seed std
$0.5$--$0.7$. Deterministic evaluation against a measured
uniform-random baseline ($n=10$ episodes, mean $-1.60$) shows
trained-condition means $0.2$--$0.9$ reward above the random
floor, but at our sample size none of the trained-vs-random
gaps reach statistical significance (Welch $t$-test
$p \geq 0.15$, one-sided Mann--Whitney $p \geq 0.08$). The
auxiliary-loss conditions do not improve on the no-aux baseline
either ($p \geq 0.23$ for both comparisons). We additionally analyse the trained
...
do stand out: (i) An RND-off ablation reveals that Random Network Distillation is the proximate cause of, not a remedy for, the hard
entropy-collapse events ($\geq 3$ documented cases), and (ii) a
crash-proof distributed pipeline that handled 10 emulator-level
...
\begin{table}[h]
  ...
  an RND-off ablation seed and a uniform-random control
  ($3$ episodes $\times$ $96$-step cap each, except random control
  at $n{=}10$). Each return is the cumulative HOU--CHI score over
  one quarter.}
  \label{tab:eval}
  \centering
  \small
  \begin{tabular}{lcccc}
    ...
    \textbf{Condition} & \textbf{Seed 0} & \textbf{Seed 1} & \textbf{Seed 2} & \textbf{Mean (std)} \\
    \midrule
    Baseline (RND on)    & ws1: $-1.67$ ($[-2,0,-3]$)  & ws2: $\phantom{-}0.00$ ($[-1,0,+1]$) & ws3: $-0.33$ ($[0,-1,0]$)     & $-0.67\;(0.85)$ \\
    OCA                  & ws4: $-2.67$ ($[-1,-4,-3]$) & ws5: $\phantom{-}0.00$ ($[0,0,0]$)   & ws6: $-1.67$ ($[-2,-1,-2]$)   & $-1.44\;(1.35)$ \\
    DPR                  & ws7: $-1.00$ ($[-1,-2,0]$)  & ws8: $-1.33$ ($[-3,+1,-2]$)          & ws10: $-0.33$ ($[+2,-2,-1]$)  & $-0.89\;(0.51)$ \\
    \midrule
    Baseline RND \emph{off} (n=1 seed) & ws8: $-1.67$ ($[-4,-2,+1]$) & --- & --- & $-1.67$ \\
    Uniform random (n=10)              & \multicolumn{3}{l}{$[-3,-5,-1,-2,-1,0,0,-2,-1,-1]$} & $-1.60\;(1.51)$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
$0.78$, slightly larger than the inter-seed std within each
condition ($0.85$, $1.35$, $0.51$); under any reasonable
...
\textbf{(iii) Pathological per-seed behaviour is visible.}
\texttt{ws5\_oca\_s1} produced three identical $0$-score
episodes: a degenerate policy that essentially never presses
(consistent with its $P(\textsc{press})=0.42$ becoming
$\arg\max=\textsc{no-press}$ for every observation in the
deterministic regime). \texttt{ws4\_oca\_s0} took the opposite
degenerate path: a single $-4$ blowout pulled its mean down.
\texttt{ws8\_dpr\_s1} produced the only positive single-episode
return ($+1$), and \texttt{ws10\_dpr\_s2} produced a $+2$.
...
$0.05$---and that recovery was observed when RND was active.
Table~\ref{tab:collapse} summarises the documented cases.

\begin{table}[h]
  \caption{Documented hard entropy-collapse events (policy entropy
  $<\!0.05$) in RND-on runs and subsequent recovery. An ablation
  with \texttt{--use-rnd} disabled (1 seed, full 312 updates)
  produced \emph{zero} such events, confirming RND as the
  proximate cause.}
  \label{tab:collapse}
  \centering
  \small
...
This causally inverts our original framing of the
collapse--recovery phenomenon. RND does not \emph{rescue} hard
entropy collapses; rather, \textbf{RND is the proximate cause of
them}. Without RND, the policy simply does not collapse. The
mechanism is most naturally explained by RND's intrinsic-bonus
gradient driving the policy toward narrow regions of action space
that achieve high RND prediction error; once concentrated, the
policy commits, entropy drops, and the same RND signal then
incentivises moving away from the converged region, raising
entropy again. The net effect on final policy quality, however,
is \emph{zero}: the RND-off ablation and the RND-on baseline
arrive at the same eval mean.

\subsection{System stability}
\label{sec:stability}

Across the training run our pipeline absorbed 10 emulator-level
...
protocol was load-bearing: 10 spontaneous emulator failures
...
coordinates and orientations. DPR is fully self-supervised on raw
...
\S\ref{sec:rnd-finding} that isolates RND's effect on entropy
collapses: the corresponding factorial cell shows RND is the
proximate cause of the collapses we documented, with no net
effect on final eval performance. The charge-duration scalar is
...
\paragraph{Wall-clock and sample size.}
...
on Android?---we report a negative answer: at our $80\,000$-step
...
all $10$ emulator-level failures and a cluster-wide kill event
...
\begin{itemize}
  \item \textbf{[Member A]}: \emph{Engineering and infrastructure.}
    Distributed training pipeline (\texttt{orchestrate.py},
    \texttt{launch\_on\_ws.sh}), crash-proof
    \texttt{EXIT/INT/TERM} trap with NFS checkpoint mirroring, adb
    backend (\texttt{adb-motionevent} with state-based DOWN/UP
    semantics), emulator AVD configuration and \texttt{clean\_boot\_v8}
    snapshot construction.
  \item \textbf{[Member B]}: \emph{Vision and reward.}
    HSV-mask + connected-components pose detector for OCA targets,
    score-event detector (\texttt{ScoreboardDiffReward}) with
    per-frame panel-colour side detection, game-over watchdog and
    blank-frame recovery, the offline P(press) charge-duration
    diagnostic (Table~\ref{tab:charge_ignored}).
  \item \textbf{[Member C]}: \emph{Algorithmic and experiments.}
    PPO + RND + charge-duration integration in \texttt{train.py},
    Agent architecture (NatureCNN encoder, auxiliary heads for OCA
    and DPR), training-curve parsing and figure generation, eval
    protocol implementation (\texttt{eval.py}), random-baseline
    control, RND-off ablation.
  \item \textbf{[Member D]}: \emph{Analysis and writing.}
    Statistical testing (CIs + Welch / Mann--Whitney), per-seed
    table assembly, entropy-collapse case-study compilation,
    methodology audits and consistency checks, paper writing and
    figure preparation.
\end{itemize}

\noindent Each section of the paper was reviewed by all members; the
listed lead is the primary contributor.
