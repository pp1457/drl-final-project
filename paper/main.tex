...
poorly understood. We take a step toward filling this gap with an
exploratory systems-level empirical study of auxiliary-representation losses
...
training-time returns cluster tightly around $-19$ (the agent's
team loses by $\approx 19$ baskets per $4$-quarter game), with
per-condition means within $0.41$ reward of each other and
inter-seed std $0.5$--$0.7$. Deterministic evaluation against a measured
...
$p \geq 0.15$, one-sided Mann--Whitney $p \geq 0.076$). The
...
do stand out: (i) a single-seed RND-off ablation that is
\emph{consistent with} Random Network Distillation being the
proximate cause of, rather than a remedy for, the hard
entropy-collapse events we observed ($4$ documented cases across
$3$ of $9$ RND-on seeds vs $0$ in the single RND-off run, Fisher
exact $p \approx 0.67$ at this sample size), and (ii) a
crash-proof distributed pipeline that handled 10 emulator-level
failures and a cluster-wide kill event without operator
intervention. Code, training logs, and evaluation JSONs are
available at \url{https://github.com/pp1457/drl-final-project}.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{(C3) A single-seed RND ablation suggestive of
        ...
        ($p \approx 0.67$), so we report this as observational
...
\begin{table}[h]
  ...
  $<\!0.05$) in RND-on runs and subsequent recovery. Lowest-entropy
  values are recomputed from the full within-update entropy traces
  of our final training run; submission-time values reported the
  per-update sample minimum, which under-sampled the true
  within-update minimum. An ablation with \texttt{--use-rnd}
...
This is suggestive evidence against our original framing of the
collapse--recovery phenomenon: it appears that RND, rather than
rescuing hard entropy collapses, is most plausibly their
proximate cause in this setup. We emphasise the limited statistical
power of a $3$-of-$9$ vs $0$-of-$1$ contingency (Fisher exact
$p \approx 0.67$); the result is consistent with --- but does not
formally establish --- a causal role for RND. The
...
returns.} Training-time returns in Table~\ref{tab:perseed} cluster
near $-19$, while deterministic per-quarter eval returns in
Table~\ref{tab:eval} cluster near $-1$. Two factors account for
the gap. (i) \emph{Horizon}: training returns accumulate over a
full $4$-quarter game ($\sim$$256$--$1024$ steps), while eval
returns are capped at one $96$-step quarter, so a per-quarter
score difference of $\sim$$-4$ to $-5$ (consistent with the
random-policy mean of $-1.60$ per quarter $\times 4 = -6.4$ per
game; the rest comes from CHI's deterministic CPU consistently
out-scoring near-random HOU play) extends to roughly $-19$ over
a full game. (ii) \emph{Stochastic vs deterministic policy}:
training rolls out the stochastic actor, while eval takes
$\arg\max$ over actor logits. For a near-Bernoulli policy
(\S\ref{sec:charge-ignored}), $\arg\max$ truncates exploration
in a way that can either accidentally help (when the modal action
matches an effective strategy) or hurt (when it locks in the
degenerate ``never press'' or ``always press'' regime visible in
Table~\ref{tab:eval}'s pathological seeds). The two regimes are
therefore not directly comparable, and neither contradicts the
other.
...
$p \approx 0.67$) --- of RND being the proximate cause of the
...
RND-off ablation seed ran the full $312$ updates. The RND-off vs
RND-on comparison in \S\ref{sec:rnd-finding} is therefore mildly
budget-mismatched (in RND-off's favour for the collapse count),
which further reinforces our reluctance to draw a formal causal
conclusion from the contingency. The per-step budget itself is a system
...
collapse?---we report the opposite of our original hypothesis. We
observe 4 hard collapses across 3 seeds in the RND-on runs; a
1-seed RND-off ablation produces zero collapses across the full 312
PPO updates while yielding the same final eval return. This is
consistent with RND being the proximate cause of, rather than a
remedy for, the collapse phenomenon in this regime, though the
$3$-of-$9$ vs $0$-of-$1$ comparison does not reach statistical
significance (Fisher exact $p \approx 0.67$) and we do not claim
formal causal attribution. The cleanest methodological takeaways
are (a) that
...
\begin{itemize}
  \item \textbf{[B12902046 Yun-Yang Liao]}: \emph{Experiments, infrastructure, and writing.}
    Distributed training pipeline and emulator orchestration
    (\texttt{orchestrate.py}, \texttt{launch\_on\_ws.sh},
    \texttt{clean\_boot\_v8} snapshot), crash-proof checkpoint
    mirroring, coordination of the 9-ws training matrix and the
    RND-off ablation, manual scoreboard verification, statistical
