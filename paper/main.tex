...
\emph{negative} one: at our planned $80\,000$-step budget (with most
seeds stopped earlier at the visible plateau; see \S\ref{sec:perseed}),
none of the three
...
exact $p \approx 0.70$ at this sample size), and (ii) a
...
$p \approx 0.70$); the result is consistent with --- but does not
...
RND-on comparison in \S\ref{sec:rnd-finding} is therefore
budget-mismatched in a direction that is \emph{conservative} for
our observation: the RND-off seed had more exposure in which a
hard collapse could have occurred (and all four RND-on collapses
happened by update $111$, well before the RND-on seeds were
stopped), so observing zero collapses over the longer run, if
anything, strengthens the directional evidence rather than
weakening it. We nevertheless retain the Fisher exact
$p \approx 0.70$ caveat: the $3$-of-$9$ vs $0$-of-$1$ seed-level
contingency is the binding statistical limitation. The per-step budget itself is a system
...
significance (Fisher exact $p \approx 0.70$) and we do not claim
