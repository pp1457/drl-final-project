...
do stand out: (i) Random Network Distillation co-occurred with recovery from hard
...
\paragraph{Bundled RND and charge-duration augmentation.} All 9
main runs use both \texttt{--use-rnd} and
\texttt{--use-charge-dim}; we therefore cannot causally attribute
either the entropy-collapse recoveries or any aspect of
training-time behaviour to RND alone or the charge-duration scalar
alone. The factorial ablation
$\{\text{RND on/off}\}\times\{\text{charge-dim on/off}\}\times
\{\text{baseline, OCA, DPR}\}$ is the most important algorithmic
follow-up.
